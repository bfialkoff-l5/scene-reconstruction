...
def test_build_summary_includes_health(tmp_path: Path) -> None:
    ...
    constants = DEFAULT_SELECTION_PARAMS.as_constants()
    health = assess_selection(candidates, DEFAULT_SELECTION_PARAMS)
...
def test_write_selection_report(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "selection_report" / "bins_map.png").is_file()
